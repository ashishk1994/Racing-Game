...
/* 
        gluLookAt(
                //  bike_x,      bike_y-0.5,      bike_z+1,
                bike_x-(2*sinf(DEG2RAD(yaw))), bike_y+1,bike_z-(2*cosf(DEG2RAD(yaw))),
                bike_x+sinf(DEG2RAD(yaw)), bike_y+1,bike_z+cosf(DEG2RAD(yaw)),
                0.0,    1.0,    0.0
                );
  */
            Set_Camera();
...
void update() 
{
	...
	{
		if(sqrt((bike_x-fossil[i].first)*(bike_x-fossil[i].first)+(bike_z-fossil[i].second)*(bike_z-fossil[i].second))<4)
		{
			score++;
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("201202100 Ashish Kumar Assignment2");
    initRendering();
